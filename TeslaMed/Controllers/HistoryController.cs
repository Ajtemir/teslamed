...
namespace TeslaMed.Controllers
{
    public class HistoryController : Controller
    {
        ...
        public HistoryController(TeslaMedContext context, IStringLocalizer<HistoryController> localizer, UserManager<User> userManager, IRepository repo, IWebHostEnvironment environment)
        {
            _context = context;
        ...
        [Authorize(Roles = "admin, manager")]
        [HttpPost]
        public async Task<IActionResult> CreateHistory(History doctor, IFormFile file)
        {
            ...
            await _repo.DbAdd<History>(doctor);
            await _repo.DbSave();
        ...
        {
            var aboutCompany = _repo.GetHistory(id);
        ...
        public async Task<IActionResult> EditHistory(History publication, IFormFile newImage, bool imageChanged)
        {
            if (publication == null)
            {
            ...
            try
            {
                var existingPublication = _repo.GetHistories()
        ...
        {
            return (_repo.GetHistories()?.Any(e => e.Id == id)).GetValueOrDefault();
        }
        [Authorize(Roles = "admin, manager")]
        public async Task<IActionResult> RemoveHistory(int id)
        {
            var doc = _repo.GetHistories().AsQueryable().FirstOrDefault(e => e.Id == id);
            if (doc == null)
            {
                return NotFound();
            }
